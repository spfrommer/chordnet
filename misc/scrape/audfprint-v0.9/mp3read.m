...
if nargin < 5
  ...
  if MPG123059
    ...
    delay = 0;
    % Hmm, delay seems to have reappeared in mpg123 1.14.4...
    % it's OK for 44k mono and stereo.  
    % it's OK for 32k mono and stereo.
    % It's even OK for 16k stereo.  But not 16k mono
    if SR == 16000 && nchans == 1
      delay = 1105;
    end
  end
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function G = protectforshell(F)
% G = protectforshell(F)
%    Insert escapes into filename F so that it can be used as an
%    argument to a shell command.
% 2013-10-09 Dan Ellis dpwe@ee.columbia.edu

%% expand tilde
if F(1) == '~'
  F = [getenv('HOME'),F(2:end)];
end

%%%%% escape weird shell characters %%%%%
badchrs = '"\`';
% need to escape "!" on Mac/Darwin, must not on Linux (??)
if ismac; badchrs = [badchrs,'!']; end
ix = find(max(repmat(F,length(badchrs),1)==repmat(badchrs',1,length(F))));
offset = 0;
for i = ix
  % Move string from "bad" character to end down one space
  F = [F,' '];
  F((i+offset)+1:end) = F((i+offset):end-1);
...
%%%% Dollar signs need very special treatment
%%%% "a$a" becomes "a"'$'"a"
ix = find(F=='$');
offset = 0;
for i = ix
  % Move string from "bad" character to end down 4 spaces
  F = [F,'    '];
  F((i+offset)+4:end) = F((i+offset):end-4);
  % insert a backslash before the "bad" character
  F(i+offset+[0:4]) = ['"',char(39),'$',char(39),'"'];
  % remember that all the subsequent characters are now 4
  % characters further down
  offset = offset+4;
end
