function [D, SR] = audioread_custom(FN, TARGETSR, FORCEMONO, START, DUR)
...
% $Header: /Users/dpwe/matlab/columbiafns/RCS/audioread.m,v 1.3 2013/10/01 14:47:20 dpwe Exp dpwe $
...
overnet = 0;
if length(FN) >= 7
  overnet = strcmp(FN(1:7), 'http://');
end

if ~overnet && exist(FN, 'file') == 0
...
try
  ...
  % if ismp3 || ism4a || iswav || isrds || isflac
    ...
    %elseif iswav
      ...
      %catch me
        %% WAV format that wavread rejects - use ReadSouand
        %SIZ = [-1 0];
        %[D,SR] = ReadSound(FN,1);
      %end
    %elseif isflac
    ...
    %elseif iswav
      ...
      %catch me
        ...
        %else
          %LEN = N(2) - N(1) + 1;
        %end
        %[D,SR] = ReadSound(FN,LEN*ds,N(1)*ds-1);
        %D = D'; % returns in rows!
      %end
    %elseif isflac
  ...
  disp(['*** audioread: problem reading ', FN, ':', me.message]);
  disp(['*** audioread: returning empty waveform']);
  D = [];
  SR = TARGETSR;
end
